...
for EXP in "${EXPERIMENTS[@]}"; do
  echo "=========================================="
  echo "🚀 Starting ${EXP} (first ${SCENE_LIMIT} scenes, 500 frames each)"
  echo "=========================================="

  # 1) streaming inference 결과 디렉토리
  INFER_PATH="${ROOT}/benchmark/output/kd_ablation/${EXP}_2scenes"
  ...
  python "${ROOT}/benchmark/eval/eval.py" \
    ...
    --wandb_run_name "${EXP}_2scenes_$(date +%Y%m%d_%H%M)" \
