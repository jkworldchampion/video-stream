        ...
        self.attention_blocks = nn.ModuleList(
            [
                TemporalAttention(
                    ...
                    temporal_max_len=temporal_max_len,  # Keep as parameter (32 from checkpoint)
                    pos_embedding_type=pos_embedding_type,
                )
...
class TemporalAttention(CrossAttention):
    ...
    def __init__(self, temporal_max_len=32, pos_embedding_type="ape", *args, **kwargs):
        ...
        # CRITICAL: Keep 32 for checkpoint compatibility (trained with 32)
        self.pos_encoder = PositionalEncoding(kwargs["query_dim"], dropout=0., max_len=32)
    ...
    def forward(
        ...
        # Positional
        # ===== CRITICAL FIX: past is already PE-applied (from cache) =====
        # Cache stores hidden_states AFTER PE application (line 382 output)
        # So we must NOT apply PE again to past!
        if self.pos_encoder is not None:
            now_pos  = self.pos_encoder(now)
            # FIXED: past already has PE applied, use as-is
        ...
        # ===== Cache management: return current frame for caching =====
        # Cache structure: DPT concatenates outputs from multiple attention blocks
        # We need to return the FULL sequence (past + now) to be used as cache in next step
        # This matches the expectation: cached_hidden_states = past (full sequence up to t-1)
        output_hidden_states = torch.cat([past, now], dim=1) if past is not None else now
        
        # ===== CRITICAL: Limit cache to temporal_max_len to prevent OOM =====
        if output_hidden_states.shape[1] > self.temporal_max_len:
            output_hidden_states = output_hidden_states[:, -self.temporal_max_len:, :]
        ...
        if return_qkv:
            return hidden_states, output_hidden_states, qkv_export
        else:
